...
# ---------------------------------------------------------------------------
section "29. CONTROLLER: plugin-cfg.xml generated by dynamicRouting setup"
# dynamicRouting setup writes this to the controller's logs/state dir
ls -la "${CTRL_SERVER}/logs/state/" 2>/dev/null >> "${OUT}"
echo "" >> "${OUT}"
cat "${CTRL_SERVER}/plugin-cfg.xml" 2>/dev/null \
    || cat "${CTRL_SERVER}/logs/state/plugin-cfg.xml" 2>/dev/null \
    || echo "(not found)" >> "${OUT}" >> "${OUT}"
